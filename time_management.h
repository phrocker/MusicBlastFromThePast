...
class Time_Management
{
	...
	public:
		...
		long time_stamp();
		string time_stamp(char *iS);
            const char *gm_time(const char *iS);
	private:
		struct tm *myTime;
	time_t long_time;
...
long Time_Management::time_stamp()
{
	return (long)long_time;
}

char *Time_Management::output_time(long iS=0)
{
	if (iS != 0)
	{
...
int
Time_Management::requested_time(int iP,long iS)
{
	struct tm *rC;
	time_t rawtime;
	time ( &rawtime );
